...
public class AggregateBinaryGPUInstruction extends GPUInstruction
{
		...
		throws DMLRuntimeException 
	{
		...
		else if(opcode.equalsIgnoreCase("tsmm")) {
			InstructionUtils.checkNumFields( parts, 4 );
			CPOperand in1 = new CPOperand(parts[1]);
			CPOperand out = new CPOperand(parts[2]);
			boolean isLeftTransposed = Boolean.parseBoolean(parts[3]);
			boolean isRightTransposed = Boolean.parseBoolean(parts[4]);
			
			AggregateOperator agg = new AggregateOperator(0, Plus.getPlusFnObject());
			AggregateBinaryOperator aggbin = new AggregateBinaryOperator(Multiply.getMultiplyFnObject(), agg, 1);
			return new AggregateBinaryGPUInstruction(aggbin, in1, null, out, opcode, str, isLeftTransposed, isRightTransposed);
		}
		else {
